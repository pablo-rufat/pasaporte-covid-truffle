...
contract PassaporteCovid {
    ...
    function cadastrarCidadao() external {
        Cidadao memory newCidadao = Cidadao(0, 0, true, address(0), 0, new string[](0));
        listaCidadao[msg.sender] = newCidadao;
    }

    // Adiciona ao array documentos o endereço do documento no ipfs
    function addDocumento(address cidadao, string calldata ipfs) external podeAdicionarDocumento() {
        listaCidadao[cidadao].documentos.push(ipfs);
    }

    // Que merda é trabalhar com arrays em solidity ein. e pra retornar elas pior
    function getDocumentos(address cidadao, uint timestamp) external view podeGetDocumentos(cidadao, timestamp) returns (string  [] memory) {
